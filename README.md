...
| `uvm-coding` | coding | **10-reference UVM 1.2 deep-dive** (Mentor Cookbook-grade): testbench architecture + factory overrides catalog, 5-tier package structure, phasing + objections mechanics, sequences (late randomization / polymorphism / library / pipelined-driver / slave-driver), virtual sequences & layering, monitor/predictor/scoreboard topology, register model (adapter / predictor / backdoor / built-in seqs / quirky regs / RAL coverage), messaging & debug plusargs, **two-kingdoms emulation split** |
